...
fetch(url)
  ...
  .then(data => {
    ...
    for (let line of uniqueLines) {
      tableRows += `<tr><td class="priority-1"><a href="getaddressbalance.html?address=${line.nodeAddress}" target=”_blank”>${line.nodeAddress}</a></td><td class="priority-4">${line.nodeIp}</td><td class="priority-4">${line.nodePort}</td><td class="priority-2">${line.countryAbbreviation}</td><td class="priority-5">${line.city}</td><td class="priority-6">${line.continuously\u0391ctive}</td><td class="priority-7">${line.latitude}</td><td class="priority-8">${line.longitude}</td></tr>`;
    }
  ...
  .catch(error => console.error(error)); // Handle errors  
    ...
    for (let line of uniqueLines) {
      tableRows += `<tr><td class="priority-1"><a href="getaddressbalance.html?address=${line.nodeAddress}" target=”_blank”>${line.nodeAddress}</a></td><td class="priority-2">${line.nodeIp}</td><td class="priority-4">${line.nodePort}</td><td class="priority-4">${line.countryAbbreviation}</td><td class="priority-5">${line.city}</td><td class="priority-6">${line.continuously\u0391ctive}</td><td class="priority-7">${line.latitude}</td><td class="priority-8">${line.longitude}</td></tr>`;
    }
